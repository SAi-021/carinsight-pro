...
// Full rupee format, e.g. 452400 -> "₹4,52,400" (inline so no external dep)
const fmtINR = v => `₹${Number(v).toLocaleString('en-IN')}`;

/* ============================================================
   CarInsight Pro — Resale Valuation
   Uses the project's real design system
   (page-wrap / ph / g2 / card / fg-fl-fi-fs / btn-gold).
   Hybrid engine: 60% ML + 40% rule-based depreciation.
   Brand/Model are real <select> dropdowns (datalist popups
   can't be themed by CSS in Chrome/Windows, so they showed
   black in day mode — selects fix that).
   ============================================================ */
...
export default function ResaleValue() {
  ...
  // When brand changes, clear the model (since model list reloads)
  const onBrandChange = (v) => setForm(f => ({ ...f, brand: v, brand_model: '' }));
        ...
        <div className="card">
          ...
          <div className="g2">
            <div className="fg">
              ...
              <select className="fs" value={form.brand} onChange={e => onBrandChange(e.target.value)}>
            ...
            <div className="fg">
              ...
              <select className="fs" value={form.brand_model}
                ...
                <option value="">
                  {!form.brand ? 'Select a brand first' : models.length === 0 ? 'No models found' : 'Select model'}
                </option>
                {models.map(m => (
